...
const interface2 = fs.readFileSync('./__contracts_ComplianceInterface_sol_ComplianceInterface.abi');
...
const deploy = async () => {
  ...
  compliance = await new web3.eth.Contract(JSON.parse(interface2)).deploy({ data: '0x' + bytecode, arguments: [picopsAddress] }).send({ from: accounts1[0], gas: '6000000' });
...
(0, _ava2.default)('Account is verified', async t => {

  beforeEach(async t => {
    await t.deploy();
  });

  _ava2.default.cb('deploys a contract', t => {
    t.pass(complianceAddress);
    t.end();
  });

  _ava2.default.cb('Checks if subscription is permitted', async t => {
  ...
  _ava2.default.cb('Checks if redemption permitted', async t => {
    const accounts = await web3.eth.getAccounts();
...
//# sourceMappingURL=/Users/kareem/melonChallenge/node_modules/.cache/ava/9459eec829cb7671795a253053cc0198.js.map