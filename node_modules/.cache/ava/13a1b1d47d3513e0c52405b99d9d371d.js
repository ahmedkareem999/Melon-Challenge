...
(0, _ava2.default)('Checks if subscription is permitted', async t => {
  ...
  const x = await compliance.methods.isSubscriptionPermitted(accounts[0], 1000000000000000000, 1000000000000000000).call();
...
(0, _ava2.default)('Checks if redemption permitted', async t => {
  const accounts = await web3.eth.getAccounts();
  await result1.methods.certify(accounts[0]).send({ from: accounts[0] });
  const y = await compliance.methods.isRedemptionPermitted(accounts[0], 1000000000000000000, 1000000000000000000).call();
  t.is(y, true, 'redemption is permitted');
});
//# sourceMappingURL=/Users/kareem/melonChallenge/node_modules/.cache/ava/13a1b1d47d3513e0c52405b99d9d371d.js.map